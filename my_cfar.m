...
% 改自Radar_Target_Generation_and_Detection.m
...
% remains contant
Range_target = [120 200 -50];
Velocity_target = [40 -20 1];
Amp_target = [1 1 0.1];
Num_target = length(Range_target);
...
for j = 1:Num_target
    for i = 1:length(t)         
        ...
        r_t(i) = Range_target(j) + t(i)*Velocity_target(j);
        ...
        Rx(i) = Amp_target(j) * cos(2*pi*(fc*(t(i)-td(i))+0.5*(t(i)-td(i))^2*slope));
...
imagesc(doppler_axis,range_axis,CUT);
% colorbar;
title('CA-CFAR Filter');
xlabel('Speed');
ylabel('Range');
